...
const PANELS = [
  { id: 'admin', label: 'Admin', icon: FiGrid, path: 'admin', color: 'orange' },
  { id: 'staff', label: 'Staff', icon: FiUsers, path: 'staff', color: 'green' },
  { id: 'user', label: 'User', icon: FiUser, path: 'user', color: 'blue' },
]
...
export default function SuperAdminPanelBar() {
  ...
  const { activeRestaurant, viewAsPanel } = useSelector((s) => s.tenant)

  const isSuperAdmin = user?.platformRole === 'superadmin' || user?.role === 'superadmin'
  if (!isSuperAdmin || !activeRestaurant?._id) return null
